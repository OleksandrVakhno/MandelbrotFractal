...
namespace fractal{
    Fractal::Fractal(int width, int height): _width(width), _height(height),
                _histogram(new int[Mandelbrot::MAX_ITERATIONS]{}), _fractal(new int[_width*_height]{}),
                _bitmap(new Bitmap(_width, _height)), _zoomlist(new ZoomList(_width, _height))
    {
    ...
    void Fractal::writeBitmap(string& name) {
        _bitmap->write(name);
    }
